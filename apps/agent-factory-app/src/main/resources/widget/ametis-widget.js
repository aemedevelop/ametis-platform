"use strict";(()=>{var g="bounce",b=["none","bounce","float","ring"],y={system:'-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif',serif:'Georgia, "Times New Roman", Times, serif',mono:'ui-monospace, SFMono-Regular, Menlo, Consolas, "Liberation Mono", monospace',humanist:'"Segoe UI", Tahoma, Geneva, Verdana, sans-serif'},f={es:{headerFallback:"Asistente",subtitleFallback:"En l\xEDnea",placeholder:"Escribe tu pregunta...",typing:"Escribiendo\u2026",errorGeneric:"No se pudo conectar con el servidor. Prueba de nuevo en unos segundos.",launcherLabel:"Abrir chat",minimizeLabel:"Minimizar chat",closeLabel:"Cerrar chat",sendLabel:"Enviar pregunta"},en:{headerFallback:"Assistant",subtitleFallback:"Online",placeholder:"Type your question...",typing:"Typing\u2026",errorGeneric:"Couldn't reach the server. Please try again in a few seconds.",launcherLabel:"Open chat",minimizeLabel:"Minimize chat",closeLabel:"Close chat",sendLabel:"Send question"}},p="#1e3a8a",h="linear-gradient(135deg, #1e3a8a 0%, #2563eb 100%)",v=`
  :host { all: initial; }
  * { box-sizing: border-box; font-family: var(--amw-font, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif); }
  ...
  .bubble {
    ...
    background: var(--amw-grad, ${h}); color: #fff; box-shadow: 0 18px 35px rgba(30,58,138,.25);
  ...
  /* Llamadas a la atenci\xF3n de la burbuja cerrada -- una de cuatro, elegida
  ...
  /* Sube y baja: flotaci\xF3n vertical suave, sin achatarse. */
  ...
  /* Timbre peri\xF3dico: quieta la mayor parte del ciclo, con un peque\xF1o
  ...
  .header { display: flex; align-items: center; justify-content: space-between; gap: 1rem; padding: .9rem 1rem; background: var(--amw-grad, ${h}); color: #fff; flex-shrink: 0; }
...
`,c={message:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>',send:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>',minus:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/></svg>',close:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>'};function w(d){let e=d;if(!e)return null;let t=e.getAttribute("data-preview")==="1",n=e.getAttribute("data-deployment")||"preview",i=e.getAttribute("data-endpoint")||"";if(!t&&(!n||!i))return null;let s=e.getAttribute("data-locale")||"es";return{publicId:n,endpoint:i.replace(/\/+$/,""),locale:f[s]?s:"es",preview:t}}function x(d,e){let t=/^#?([0-9a-f]{3}|[0-9a-f]{6})$/i.exec(d.trim());if(!t)return d;let n=t[1];n.length===3&&(n=n.split("").map(o=>o+o).join(""));let i=parseInt(n,16),s=o=>Math.round(o+(255-o)*e),a=s(i>>16&255),r=s(i>>8&255),l=s(i&255);return`#${(1<<24|a<<16|r<<8|l).toString(16).slice(1)}`}var m=class{constructor(e){this.open=!1;this.bubbleAnimation=g;this.loaded=!1;this.sending=!1;this.suggestedQuestions=[];this.suggestionCount=3;this.suggestionOrder="random";this.publicId=e.publicId,this.endpoint=e.endpoint,this.strings=f[e.locale],this.preview=e.preview}mount(){let e=document.createElement("div");e.style.all="initial",e.setAttribute("data-ametis-widget-host",this.publicId),document.body.appendChild(e),this.shadow=e.attachShadow({mode:"open"});let t=document.createElement("style");t.textContent=v,this.shadow.appendChild(t);let n=document.createElement("div");n.className="root",n.innerHTML=`
