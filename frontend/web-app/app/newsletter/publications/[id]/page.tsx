...
export default function NewsletterPublicationDetailPage() {
  const t = useT();
  const params = useParams();
  const publicationId = String(params.id);
  const publicationQuery = useQuery({ queryKey: ["newsletter", "publication", publicationId], queryFn: () => fetchPublication(publicationId) });
