...
- **Chat widget** (`frontend/ametis-widget/`, new package): vanilla TS + Shadow DOM, esbuild → `apps/agent-factory-app/src/main/resources/widget/ametis-widget.js` (regen with `npm run build:copy`). Served by `PublicWidgetController` at `/api/agent-factory/public/widget.js`. Design ported 1:1 from `am-landing-react/src/components/AmetisChatWidget/`. `DeploymentEndpoints` `widget-url` derives from `public.base-url`. **Verified end-to-end locally** (`demo.html` + `node serve.js` against a real `WEB_CHAT` deployment → welcome message, real RAG answer, suggestion chips). Backend compiles.
  ...
  - **Contenido personalizado del asistente** (V18 `suggested_questions`+`assistant_texts`; V19 `_count`+`_order`; V20 `question_topics` jsonb):
    - **Preguntas sugeridas**: fondo "General" (lista plana) + **grupos por tema** (`{id,label,questions}`). `count` + `order` (`random`/`fixed`). Textos opcionales que sustituyen los genéricos (`fallback`/`greeting`/`thanks`/`farewell`/`help`).
    - **Feature B implementada con Qdrant**: al guardar/borrar el agente, `AgentService` llama `PUT|DELETE /agents/{id}/suggestion-index` (best-effort, no bloquea el guardado) → el RAG embebe cada pregunta e indexa en colección por tenant `…_suggested_questions` (payload `agent_id`, `topic_id`). En `/query` el RAG reutiliza el vector de la pregunta del retrieval, casa el tema más cercano (`SUGGESTED_QUESTIONS_MATCH_THRESHOLD`, def 0.62) y devuelve `matched_topic_id`. La plataforma (`PublicDeploymentService.followUpPool`) pasa en `suggestions` el pool del tema (o "General"); el widget muestra el subconjunto (`count`/`order`, excluye la pregunta hecha). Bienvenida = "General".
    - RAG: `build_suggestions()`→`[]`, `match_prebuilt_response()` devuelve la clave del tipo, `QueryResponse.prebuilt_key` + `matched_topic_id`. `max_output_tokens`→`LLM_MAX_OUTPUT_TOKENS` (800). `ensure_agent_definitions_name_index()` corrige el índice de nombre a `(tenant,business,lower(name))` y borra fila obsoleta al sincronizar (fix del 500 `UniqueViolation`).
    - Rebuild: `agent-factory-app` + `agent-factory-web` + `rag-service` + widget. Migraciones a **V20**.
    - Escala futura (v3+): mover el matching de tema a Redis/colección Qdrant compartida detrás de una interfaz `TopicEmbeddingStore` (hoy: colección Qdrant por tenant, embeddings recalculados solo al guardar).
