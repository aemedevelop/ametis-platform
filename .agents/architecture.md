...
- **Contenido personalizado del asistente** (`agents.suggested_questions`, `agents.assistant_texts` V18; `agents.suggested_questions_count`, `agents.suggested_questions_order` V19 — **no viaja al RAG**): el cliente configura por agente las preguntas sugeridas que quiera, cuántas se muestran cada vez (`count`) y si salen `random` o `fixed` (chips del widget: bienvenida + follow-up; el widget elige el subconjunto), más textos que sustituyen los mensajes genéricos (`fallback`, `greeting`, `thanks`, `farewell`, `help`). `GET /public/{publicId}` devuelve `suggestedQuestions`. En `/query` la plataforma, según `response_type`/`prebuilt_key` que devuelve el RAG, cambia `answer` por el texto del cliente y `suggestions` por su lista. El RAG dejó de tener sugerencias hardcodeadas (`build_suggestions` → `[]`); `match_prebuilt_response` devuelve además la clave del tipo.
